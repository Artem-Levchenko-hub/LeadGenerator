...
class Settings(BaseSettings):
    ...
    # Квота: сколько глубоких Outreach-анализов запускаем за день. По
    # бюджету ₽2000/мес и стратегии «качество > количество»: 5 × ~₽10 =
    # ₽50/день = ₽1500/мес. Каждый из этих 5 — серьёзный персональный
    # research, не шаблон.
    daily_outreach_quota: int = 5

    # Минимальный score чтобы лид был достоин deep-analysis. Считается
    # scorer'ом (без LLM) на основе ICP, наличия сайта, CMS, mobile,
    # https и т.д. 0=холодный, 100=горящий. Из 5/день берём только реально
    # горячие.
